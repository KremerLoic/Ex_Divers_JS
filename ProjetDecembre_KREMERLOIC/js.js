...
var jour   = ["Lundi","Mardi","Mercredi","Jeudi","Vendredi","Samedi","Dimanche"];   // Déclaration des jours de la semaine
...
for (var i = 0 ; i < jour.length ; i++){
	...
	if (temp[i] > 25 || temp[i] < -5 || isNaN(temp[i])){
		do {
			temp[i] = parseInt(prompt("Quelle est la temperature pour " +jour[i]));
		} while ( temp[i] < -5 || temp[i] > 25 || isNaN(temp[i])) ;	
...
if (  k > temp[i] && k % 5 === 0 ){
	...
	console.log(  jour[i].substr(0,1)+' ' +star +barre +' // Temperature = '  +temp[i] +'°'); 
}
